...
import { EnvelopeEncryption, generateMasterKey, TxSecureRecord } from "@mirfa/crypto";
import { TxStore } from "./store";
import { EncryptRequest, EncryptResponse, DecryptResponse, ErrorResponse } from "./types";
...
app.post<{ Body: EncryptRequest }>(
  ...
  async (request, reply) => {
    try {
      ...
      return reply.status(201).send(record as EncryptResponse);
    } catch (error) {
      app.log.error(error);
...
app.post<{ Params: { id: string } }>(
  ...
  async (request, reply) => {
    try {
      ...
      return reply.send({ payload: result } as DecryptResponse);
    } catch (error) {
      app.log.error(error);
      return reply.status(500).send({
